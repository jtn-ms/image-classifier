...
filecounts = [2489,2267,2317,2346,2326,2312,2325,2002,1911,2129]#[3,3,3,3,3,3,3,3,3,3]#
...
class Recognizer(caffe.Net):
    ...
    def predict(self, img_f, oversample=True):
        """
        ...
        inputs =[caffe.io.load_image(img_f)]
    ...
    def get_features(self,imgfile):
        #print "getting features for", file
        scores,fc7 = self.predict(imgfile)
        fc7 = fc7.reshape(fc7.shape[1],)
        return fc7
...
def get_dataset(net):
    features,labels = [],[]
    db_dir = '/media/frank/Data/Database/ImageNet/Kaggle/train/'#train/'#
    filelist_path = './filelist/'
    ext = '.txt'
    for i in range(len(filecounts)):
        images = glob.glob(db_dir + subdirs[i] + "/*.jpg")
        feature2 = map(lambda f: net.get_features(f), images)
